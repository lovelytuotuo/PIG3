...
void HAL_I2C_MemTxCpltCallback(I2C_HandleTypeDef *hi2c)
{
    ...
    else if (whatToDo == 3)
    {
        ...
        dat = 0x00;
        HAL_I2C_Mem_Write_IT(&hi2c2, OLED_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT, &dat, 1);
        whatToDo = 0;
    }
    else if (whatToDo == 2)
    {
    ...
    {
        HAL_I2C_Mem_Write_IT(&hi2c2, OLED_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT, (uint8_t *)(&oled_cache[y]), 128);
        y += 1;
        if (y > 7)
        {
            y = 0;
...
{
    if (cmd == OLED_CMD)
        HAL_I2C_Mem_Write(&hi2c2, OLED_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT, &dat, 1, 50);
    else
    {
        // HAL_I2C_Mem_Write(&hi2c2, OLED_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT, &dat, 1, 5);
        // // HAL_I2C_Mem_Write_IT(&hi2c2, OLED_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT, &dat, 1);
